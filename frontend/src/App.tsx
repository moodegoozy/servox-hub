...
// إعدادات ربط البوت — تُخزّن في app_settings/whatsapp (Firestore المحمي بتسجيل الدخول)
const WA_AUTO_DEFAULTS = {
  waToken: '',    // مفتاح whatsbot — يُدخل مرة في الإعدادات
  waInstance: '', // معرّف المثيل
};
type WaAutoConfig = typeof WA_AUTO_DEFAULTS;
...
function App() {
  ...
  const waConfigured = !!(waAuto.waToken && waAuto.waInstance); // المفتاح والمثيل مُدخلان // بحث بالاسم أو الجوال أو IP
    ...
    <div className="container">
      ...
      {showEditModal && editingCustomer && (
        ...
        {activeTab === 'whatsapp' && (() => {
          ...
          // المبلغ الفعلي للعميل: المبلغ الموحّد إن كُتب، وإلا قيمة اشتراكه
          const amountOf = (c: Customer): number => {
            const override = parseFloat(waAmount.trim());
            if (waAmount.trim() && !isNaN(override)) return override;
            return c.subscriptionValue ?? 0;
          };
          // فاتورته صفر ⇒ لا شيء عليه، فلا يُعتبر «سدد» ولا يُرسل له
          const isZeroInvoice = (c: Customer) => amountOf(c) <= 0;
          const statusMatch = (c: Customer) => {
            if (waStatusFilter === 'all') return true;
            // من فاتورته صفر لا يظهر ضمن «سدد»
            if (waStatusFilter === 'paid' && isZeroInvoice(c)) return false;
          ...
          const isBlocked = (c: Customer) => !!c.isSuspended || isZeroInvoice(c);
          ...
          const zeroCount = waCustomers.filter(c => !c.isSuspended && isZeroInvoice(c)).length;
          // إرسال يدوي للمحددين عبر البوت مباشرة (بضغطة واحدة)
          const sendSelectedViaBot = async () => {
            if (!waConfigured) { setToastMessage('اضبط ربط البوت أولاً (المفتاح والمثيل)'); return; }
            const targets = selectedCustomers.filter(c => normalizePhone(c.phone)).map(c => ({ phone: c.phone!, msg: buildMsg(c) }));
            if (targets.length === 0) { setToastMessage('لا يوجد محدَّدون بأرقام صالحة'); return; }
            setWaAutoBusy('bulk');
            const { sent, lastError } = await sendWaBatch(targets);
            setWaAutoBusy('');
            setToastMessage(sent === 0 && lastError ? `❌ فشل الإرسال: ${lastError}` : `📤 أُرسلت ${sent} من ${targets.length} عبر البوت`);
          };
          ...
          <div className="section wa-section">
            ...
            {/* ربط البوت (اختياري — للإرسال المباشر للمحددين) */}
            <div className="wa-panel wa-auto-panel">
              <div className="wa-auto-head">
                <div className="wa-panel-title">🔌 ربط البوت (اختياري)</div>
                <span className={`wa-backend-badge ${waConfigured ? 'ok' : 'off'}`}>
                  {waConfigured ? '✅ مربوط' : '⚠️ غير مربوط'}
                </span>
              </div>
              <p className="wa-auto-warn" style={{ marginBottom: 12 }}>اربط البوت لإرسال الرسائل للمحددين مباشرة بضغطة واحدة. أو اترك الحقول فارغة واستخدم زر «واتساب» بجانب كل عميل لفتح المحادثة يدوياً.</p>
              <div className="wa-auto-templates">
                ...
                <div className="wa-auto-field">
                  ...
                  <input type="text" value={waAuto.waInstance} onChange={(e) => setWaAuto({ ...waAuto, waInstance: e.target.value })} placeholder="مثال: 6A66EC7B83716" dir="ltr" />
                </div>
              </div>
            ...
            {selectedCustomers.length > 0 && (
              <div className="wa-send-bar">
                {waConfigured ? (
                  <>
                    <button className="wa-bulk-btn" disabled={!!waAutoBusy} onClick={sendSelectedViaBot}>
                      {waAutoBusy === 'bulk' ? 'جارٍ الإرسال...' : `📤 إرسال للمحددين عبر البوت (${selectedCustomers.length})`}
                    </button>
                    <span className="wa-send-hint">تُرسَل الرسالة مباشرة عبر البوت لكل عميل محدَّد (بتباعد بسيط).</span>
                  </>
                ) : (
                  <span className="wa-send-hint">لإرسال جماعي بضغطة واحدة اربط البوت بالأعلى، أو استخدم زر «واتساب» بجانب كل عميل.</span>
                )}
              </div>
            )}
              ...
              <>
                ...
                <div className="wa-list">
                  {waCustomers.map(c => {
                    ...
                    const zeroInvoice = !suspended && isZeroInvoice(c);
                    ...
                    return (
                      <div key={c.id} className={`wa-row ${selectedSet.has(c.id) && !blocked ? 'selected' : ''} ${suspended ? 'suspended' : ''} ${zeroInvoice ? 'zero-invoice' : ''}`}>
                        ...
                        {suspended ? (
                          ...
                          <span className="wa-zero-badge" title="لا يوجد مبلغ مستحق — لا يُرسل له">فاتورته 0</span>
                        ) : hasPhone ? (
